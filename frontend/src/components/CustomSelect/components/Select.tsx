...
const Select: React.FC<SelectProps> = ({
  ...
  const getSelectClass = useCallback(() => {
    ...
    const baseClass = "flex text-sm text-black border border-transparent rounded shadow-sm transition-all duration-300 focus:outline-none";
    const defaultClass = `${baseClass} ${isDisabled ? "bg-gray-200" : `bg-white hover:border-transparent ${borderFocus} focus:ring ${ringColor}`}`;
  ...
  const getTagItemClass = useCallback(
    (item: Option) => {
      ...
      const disabledClass = isDisabled ? "border-transparent px-1" : "pl-1";
      return classNames?.tagItem ? classNames.tagItem({ item, isDisabled }) : `${baseClasse} ${disabledClass}`;
    },
  ...
  return (
    ...
    >
      <div className="relative w-full" ref={ref}>
        <div aria-expanded={open} onKeyDown={onPressEnterOrSpace} onClick={toggle} className={getSelectClass()}>
          <div className="flex grow flex-wrap gap-1 py-2 pl-2.5 pr-2">
            {!isMultiple ? (
              <p className="cursor-default select-none truncate">{value && !Array.isArray(value) ? value.label : placeholder}</p>
            ) : (
              <>
                ...
                {Array.isArray(value) &&
                  value.map((item, index) => (
                    <div className={getTagItemClass(item)} key={index}>
                      <p className={classNames?.tagItemText ? classNames.tagItemText : "cursor-default select-none truncate text-gray-600"}>{item.label}</p>
                      {!isDisabled && (
                        <div
                          role="button"
                          tabIndex={0}
                          onClick={(e) => removeItem(e, item)}
                          className={classNames?.tagItemIconContainer ? classNames.tagItemIconContainer : "flex cursor-pointer items-center rounded-r-sm px-1 hover:bg-red-200 hover:text-red-600"}
                        >
                          <CloseIcon className={classNames?.tagItemIcon ? classNames.tagItemIcon : "mt-0.5 size-3"} />
                        </div>
                      )}
                    </div>
                  ))}
          ...
          <div className="flex flex-none items-center py-1.5">
            ...
            {isClearable && !isDisabled && value !== null && (
              <div className="cursor-pointer px-1.5" onClick={clearValue}>
                <CloseIcon className={classNames?.closeIcon ? classNames.closeIcon : "size-5 p-0.5"} />
              </div>
            )}
            ...
            <div className="px-1.5">
              <ChevronIcon className={`size-6 p-0.5 transition duration-300${open ? " rotate-90 text-gray-500" : " text-gray-500"}`} />
            </div>
          </div>
        </div>

        {open && !isDisabled && (
          <div className={classNames?.menu ? classNames.menu : "absolute z-10 mt-1.5 w-full rounded border bg-white py-1 text-sm text-gray-700 shadow-lg"}>
            {isSearchable && (
              <SearchInput
                ref={searchBoxRef}
                value={inputValue}
            ...
            <Options list={list} noOptionsMessage={noOptionsMessage} text={inputValue} isMultiple={isMultiple} value={value} primaryColor={primaryColor || DEFAULT_THEME} />
          </div>
        )}
      </div>
    </SelectProvider>
