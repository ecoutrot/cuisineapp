...
function RecipeImproveGeneratorForm({ recipe, ingredients, units, recipeCategories }: { recipe: Recipe; ingredients: Ingredient[]; units: Unit[]; recipeCategories: RecipeCategory[] }) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (recipeIngredients != null) {
      for (let i = 0; i < recipeIngredients.length; i++) {
        const id = toStringOrNull(formData.get(`recipeIngredients[${i}].id`) as string);
        const unitId = toStringOrNull(formData.get(`recipeIngredients[${i}].unitId`) as string);
        const quantity = toFloatOrNull(formData.get(`recipeIngredients[${i}].quantity`) as string);
        const ingredientId = toStringOrNull(formData.get(`recipeIngredients[${i}].ingredientId`) as string);
        ...
        <div className="flex items-center justify-between">
          ...
          {recipe?.id && (
            <Link
              ...
              className="ml-4 rounded-lg border border-green-600 bg-green-600 p-2.5 px-5 text-sm font-medium text-white shadow hover:bg-transparent hover:text-green-600 focus:outline-none focus:ring-2 focus:ring-green-600 active:text-green-500"
            >
        ...
        {recipeIngredients.map((recipeIngredient, index) => (
          <div key={nanoid()} className="gap-4 md:flex">
            <div className="flex gap-4">
              <input type="hidden" name={`recipeIngredients[${index}].id`} defaultValue={recipeIngredient.id ?? ""} />
              <CuisineInputNumber
...
export default RecipeImproveGeneratorForm;
